<header x-data="{ open: false, dropdownProfil: false, dropdownAlumni: false, dropdownInfo: false }" class="fixed top-0 left-0 right-0 z-50 glass-header transition-all duration-300">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between h-20">
            ...
            <nav class="hidden md:flex items-center space-x-1 lg:space-x-2">
                ...
                <!-- 4. Dropdown Informasi & Media -->
                <div class="relative" @mouseenter="dropdownInfo = true" @mouseleave="dropdownInfo = false">
                    <button type="button" @click="dropdownInfo = !dropdownInfo" class="px-3.5 py-2 rounded-xl text-sm font-semibold transition-all duration-200 flex items-center {{ request()->routeIs('berita.*', 'beasiswa.*', 'infografis.*', 'galeri') ? 'bg-slate-900 text-white shadow-sm' : 'text-slate-700 hover:text-slate-900 hover:bg-slate-100' }}">
                        <i class="fa-solid fa-newspaper mr-1.5 opacity-70"></i>Informasi & Media
                        <i class="fa-solid fa-chevron-down ml-1.5 text-xs opacity-60 transition-transform duration-200" :class="{ 'rotate-180': dropdownInfo }"></i>
                    </button>
                    <div x-show="dropdownInfo" 
                         x-transition:enter="transition ease-out duration-150" 
                         x-transition:enter-start="opacity-0 translate-y-1" 
                         x-transition:enter-end="opacity-100 translate-y-0" 
                         x-transition:leave="transition ease-in duration-100" 
                         class="absolute left-0 mt-1 w-56 rounded-2xl bg-white/95 backdrop-blur-md shadow-xl border border-slate-200/80 p-2 z-50 space-y-1" 
                         style="display: none;">
                        <a href="{{ route('berita.index') }}" class="flex items-center px-3 py-2 rounded-xl text-xs font-semibold text-slate-700 hover:bg-slate-100 hover:text-slate-900 {{ request()->routeIs('berita.*') ? 'bg-slate-100 font-bold text-slate-900' : '' }}">
                            <i class="fa-solid fa-newspaper w-5 text-emerald-500"></i>Berita & Kegiatan
                        </a>
                        <a href="{{ route('beasiswa.index') }}" class="flex items-center px-3 py-2 rounded-xl text-xs font-semibold text-slate-700 hover:bg-slate-100 hover:text-slate-900 {{ request()->routeIs('beasiswa.*') ? 'bg-slate-100 font-bold text-slate-900' : '' }}">
                            <i class="fa-solid fa-graduation-cap w-5 text-amber-500"></i>Program Beasiswa
                        </a>
                        <a href="{{ route('infografis.index') }}" class="flex items-center px-3 py-2 rounded-xl text-xs font-semibold text-slate-700 hover:bg-slate-100 hover:text-slate-900 {{ request()->routeIs('infografis.*') ? 'bg-slate-100 font-bold text-slate-900' : '' }}">
                            <i class="fa-solid fa-bullhorn w-5 text-rose-500"></i>Infografis & Flyer
                        </a>
                        <a href="{{ route('galeri') }}" class="flex items-center px-3 py-2 rounded-xl text-xs font-semibold text-slate-700 hover:bg-slate-100 hover:text-slate-900 {{ request()->routeIs('galeri') ? 'bg-slate-100 font-bold text-slate-900' : '' }}">
                            <i class="fa-solid fa-images w-5 text-sky-500"></i>Galeri Foto
                        </a>
                    </div>
                </div>
            </nav>
    ...
    <div x-show="open" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 -translate-y-4" x-transition:enter-end="opacity-100 translate-y-0" x-transition:leave="transition ease-in duration-150" class="md:hidden bg-white border-b border-slate-200 px-4 pt-2 pb-6 space-y-2">
        ...
        <!-- Mobile Accordion Informasi & Media -->
        <div x-data="{ subOpen: false }">
            <button @click="subOpen = !subOpen" class="w-full flex items-center justify-between px-3 py-2.5 rounded-xl text-base font-semibold text-slate-800 hover:bg-slate-100">
                <span><i class="fa-solid fa-newspaper w-6 text-slate-500"></i>Informasi & Media</span>
                <i class="fa-solid fa-chevron-down text-xs text-slate-400 transition-transform" :class="{ 'rotate-180': subOpen }"></i>
            </button>
            <div x-show="subOpen" class="pl-8 space-y-1 pt-1">
                <a href="{{ route('berita.index') }}" class="block px-3 py-2 rounded-xl text-sm font-medium text-slate-700 hover:bg-slate-100">Berita & Kegiatan</a>
                <a href="{{ route('beasiswa.index') }}" class="block px-3 py-2 rounded-xl text-sm font-medium text-slate-700 hover:bg-slate-100">Program Beasiswa</a>
                <a href="{{ route('infografis.index') }}" class="block px-3 py-2 rounded-xl text-sm font-medium text-slate-700 hover:bg-slate-100">Infografis & Flyer</a>
                <a href="{{ route('galeri') }}" class="block px-3 py-2 rounded-xl text-sm font-medium text-slate-700 hover:bg-slate-100">Galeri Foto</a>
            </div>
        </div>
